...
from .models import Product, Subcategory, Category, Rate
...
class ProductView(GenericViewSet, ListModelMixin):
    renderer_classes = [MyRenderer, JSONRenderer]
    queryset = qs = Product.objects.all().annotate(
        average_rate=Avg('rates__rate')
    ).order_by(
        F('average_rate').desc(nulls_first=True), '-create_date'
    )
    ...
    search_fields = ('title',)
    serializers = {
        'list': se.ProductSerializer,
        'retrieve': se.ProductDetailSerializer,
    ...
    def retrieve(self, request, **kwargs):
        try:
            instance = Product.objects.filter(pk=kwargs['pk']).prefetch_related(
                'viewed_by', 'medias__special_users', 'special_users', 'store__special_users'
            )[0]
        except ObjectDoesNotExist:
            return Response({'error': 'the product by id = {} does not exist'.format(kwargs['pk'])})
        ...
        if 'Authorization' in request.COOKIES:
            ...
            if instance.price != 0 or False in [media.price != 0 for media in instance.medias.all()]:
                if user in instance.special_users.all() or user in instance.store.special_users.all():
                    product = se.ProductDetailSerializer(instance, context={'request': request})
                    files = se.MediaSerializer(instance.medias, many=True, context={'request': request})
                    return Response({'product info': product.data, 'files': files.data, 'hide_files': []})
                else:
                    product = se.ProductDetailSerializer(instance, context={'request': request})
                    files = []
                    hide_files = []
                    ...
                    return Response({'product info': product.data, 'files': files, 'hide_files': hide_files})
            else:
                product = se.ProductDetailSerializer(instance, context={'request': request})
                files = se.MediaSerializer(instance.medias, many=True, context={'request': request})
                return Response({'product info': product.data, 'files': files.data, 'hide_files': []})
    ...
    def buy(self, request, **kwargs):
        if 'Authorization' in request.COOKIES:
            ...
            return HttpResponseRedirect(reverse('product-detail', kwargs={'pk': kwargs['pk']}))
        else:
            return HttpResponseRedirect(reverse('login'))


def media_buy(request, product_id, media_id):
    if 'Authorization' in request.COOKIES:
        buy_media.delay(get_user_by_token(request), media_id)
        return HttpResponseRedirect(reverse('product-detail', kwargs={'pk': product_id}))
    else:
        return HttpResponseRedirect(reverse('login'))
